...
module.exports.run = async (bot, message, args) => {//exportando o comando como ship
 ...
 let nomeship;
  ...
  let emoticon;
    if(amor > 60) {
      emoticon = ("https://cdn.discordapp.com/attachments/815212696214700042/821043513562103868/unknown.png"); //imagem de coração
    } else if(amor >= 40) {
  ...
  let desc;
    if(amor > 90) {
      desc = (":sparkling_heart: Será? :sparkling_heart:\n```"+membro1.user.username+"```\n```"+membro2.user.username+"```\n:heart: ```"+nomeship+"``` Esse é o casal perfeito! :heart:");
    } else if(amor >= 70) {
      desc = (":sparkling_heart: Será? :sparkling_heart:\n```"+membro1.user.username+"```\n```"+membro2.user.username+"```\n:neutral_face: ```"+nomeship+"``` Esses Daqui já São Casados  :scream_cat:");
    } else if(amor >= 45) {
